  ...
  return (
    <motion.span
      ...
      animate={{
        ...
        color: isHovered ? "hsl(var(--accent-foreground))" : isHighlightable ? "hsl(var(--accent))" : "hsl(var(--foreground))",
    ...
    >
      <AnimatePresence>
        {isHovered && (
          <motion.span
            className="absolute -inset-2 rounded-lg bg-accent z-[-1]"
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0, scale: 0.8 }}
            layoutId="hover-bg"
            style={{
              boxShadow: "0px 10px 25px -5px hsl(var(--accent) / 0.4), 0px 8px 10px -6px rgba(0, 0, 0, 0.1)",
            }}
          />
        )}
      </AnimatePresence>
        ...
        {isHovered && (
          <>
            ...
            <motion.span
              ...
              animate={{ scale: 1 }}
              exit={{ scale: 0 }}
              className="absolute -bottom-1 -left-1 w-2 h-2 bg-primary rounded-full z-20"
            />
          </>
        )}
      </AnimatePresence>
